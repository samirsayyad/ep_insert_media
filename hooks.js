...
const s3UploadMedia = (file, savedFilename, fileType) => {
  ...
  s3.upload(paramsUpload, (err, data) => {
    ...
    if (data) {
      return {type: settings.ep_insert_media.storage.type, error: false, fileName: savedFilename, fileType, data};
    } else {
  ...
  context.app.post('/p/:padId/pluginfw/ep_insert_media/upload', (req, res, next) => {
    try {
      ...
      busboy.on('file', (fieldname, file, filename, encoding, mimetype) => {
        ...
        const uploadResult = (settings.ep_insert_media.storage.type === 's3')
